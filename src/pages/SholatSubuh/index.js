...
import { Takbiratulihram } from '../../assets/audio';
import NiatSubuh from '../../assets/audio/NiatSubuh.mp3';

const windowHeight = Dimensions.get('window').height;

const SholatSubuh = () => {
  const [isPlayingNiat, setIsPlayingNiat] = useState(false);
  const [isPlayingTakbiratulihram, setIsPlayingTakbiratulihram] =
    useState(false);

  const soundNiatSubuh = useRef(
    new Sound(NiatSubuh, Sound.MAIN_BUNDLE, error => {
      if (error) {
  ...
  const soundTakbiratulihram = useRef(
    new Sound(Takbiratulihram, Sound.MAIN_BUNDLE, error => {
      if (error) {
        console.log('Gagal memuat suara:', error);
      }
    }),
  ...
  const playSoundNiatSubuh = async () => {
    if (isPlayingNiat) {
      try {
        await soundNiatSubuh.current.pause();
        setIsPlayingNiat(false);
      } catch (e) {
        console.log('Gagal menghentikan audio niat:', e);
      }
    } else {
      try {
        await soundNiatSubuh.current.play(() => {
          setIsPlayingNiat(false);
        });
        setIsPlayingNiat(true);
        // Pastikan audio Takbiratulihram dimatikan jika masih bermain
        if (isPlayingTakbiratulihram) {
          try {
            await soundTakbiratulihram.current.pause();
            setIsPlayingTakbiratulihram(false);
          } catch (e) {
            console.log('Gagal menghentikan audio Takbiratulihram:', e);
          }
        }
      } catch (e) {
        console.log('Gagal memutar audio niat:', e);
      }
    }
  };

  const playSoundTakbiratulihram = async () => {
    if (isPlayingTakbiratulihram) {
      try {
        await soundTakbiratulihram.current.pause();
        setIsPlayingTakbiratulihram(false);
      ...
      try {
        await soundTakbiratulihram.current.play(() => {
          setIsPlayingTakbiratulihram(false);
        });
        setIsPlayingTakbiratulihram(true);
        // Pastikan audio niat dimatikan jika masih bermain
        if (isPlayingNiat) {
          try {
            await soundNiatSubuh.current.pause();
            setIsPlayingNiat(false);
        ...
        console.log('Gagal memutar audio Takbiratulihram:', e);
      }
    }
  };
      ...
      <View style={styles.contentContainer}>
        <ImageBackground source={Icon} style={styles.IconImage}>
          <View style={styles.header}>
            <Image source={HeaderSholatSubuh} style={styles.headerImage} />
          </View>
        </ImageBackground>

        <View style={styles.imageContainer}>
          <Image source={DefinisiSholatSubuh} style={styles.image} />
        </View>
        ...
        <View style={styles.imageContainer}>
          <ImageBackground source={KotakNiat} style={styles.imageKotakNiat}>
            ...
            <Text style={styles.bacaanniatText}>Bacaan Niat Sholat Subuh:</Text>
            <Text style={styles.ArabniatText}>
              اُصَلّى فَرْضَ الصُّبْحِ رَكْعَتَيْنِ مُسْتَقْبِلَ الْقِبْلَةِ
              اَدَاءً للهِ تَعَالَى .
            </Text>
            <Text style={styles.Artiniat}>
              Artinya: “aku niat sholat fardhu subuh dua rakaat menghadap kiblat
              karena Allah ta’ala.”
            </Text>
            <TouchableHighlight
              onPress={playSoundNiatSubuh}
              underlayColor="transparent">
              <View
                style={[
                  styles.soundButton,
                  isPlayingNiat && styles.stopButton,
                  !isPlayingNiat && styles.playButton,
                ]}>
                <Text style={styles.soundButtonText}>
                  {isPlayingNiat ? 'Stop' : 'Play'}
                </Text>
              </View>
            </TouchableHighlight>
          </ImageBackground>
        ...
        <View style={styles.imageContainer2}>
          <ImageBackground source={KotakNiat} style={styles.imageKotakNiat2}>
            ...
            <TouchableHighlight
              onPress={playSoundTakbiratulihram}
              underlayColor="transparent">
              <View
                style={[
                  styles.soundButtonTakbiratulihram,
                  isPlayingTakbiratulihram && styles.stopButtonTakbiratulihram,
                  !isPlayingTakbiratulihram && styles.playButtonTakbiratulihram,
                ]}>
                <Text style={styles.soundButtonTextTakbiratulihram}>
                  {isPlayingTakbiratulihram ? 'Stop' : 'Play'}
                </Text>
              </View>
            </TouchableHighlight>
          </ImageBackground>
...
const styles = StyleSheet.create({
  ...
  soundButtonTextTakbiratulihram: {
    color: 'white',
    fontWeight: 'bold',
  },
  stopButtonTakbiratulihram: {
    backgroundColor: '#FFD700',
  },
  playButtonTakbiratulihram: {
    backgroundColor: '#058D1A',
  },
  ...
  soundButtonTextDoaiftitah: {
    color: 'white',
    fontWeight: 'bold',
  },
  stopButtonDoaiftitah: {
    backgroundColor: '#FFD700',
  },
  playButtonDoaiftitah: {
    backgroundColor: '#058D1A',
  },
